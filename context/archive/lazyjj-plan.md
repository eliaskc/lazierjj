# kajji - A Modern TUI for Jujutsu
...
| **Config format** | TOML | Match jj's config format, read from `[kajji]` section in jj config |
...
1. `[kajji]` section in jj's config (`~/.jjconfig.toml` or `.jj/repo/config.toml`)
2. Standalone `~/.config/kajji/config.toml` (fallback, respects XDG_CONFIG_HOME)
...
1. **kajji config** - If `diff_tool` is set in `[kajji]`, use `jj diff --tool <configured>`
